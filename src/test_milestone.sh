...
do
    ...
    then
        ...
        ./client < tests/test${TEST_ID}gen.dsl 2> outputs/test${TEST_ID}gen.out.err 1> outputs/test${TEST_ID}gen.out

        sleep 1

        # run the "comparison" script for comparing against expected output for test_id
        bash verify_output_standalone.sh $TEST_ID outputs/test${TEST_ID}gen.out tests/test${TEST_ID}gen.exp outputs/test${TEST_ID}gen.cleaned.out outputs/test${TEST_ID}gen.cleaned.sorted.out
